...
The MCP path is relative to `rpc.toml`; the stdio server `cwd` is relative to `mcp.json`. Replace the placeholder `docs-mcp` command with an installed MCP server. The optional `prefix` changes model-facing tool names without changing the server identity; use `""` for native names. Keep credentials in process environment variables or an external secret manager rather than in `mcp.json`.
