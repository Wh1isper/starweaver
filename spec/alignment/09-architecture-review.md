...
- `LocalEnvironmentProvider` canonicalizes physical read targets before enforcing allowed-root containment. Write and creation paths reject every pre-existing symlink component before directory creation or mutation, including provider-managed scratch files; Unix adversarial tests cover external-root reads, writes, shell cwd selection, scratch-file writes, and symlink-entry move/delete semantics. The local provider documents its intentionally non-sandboxed threat model: hosts needing resistance to a concurrent untrusted filesystem writer must use a sandboxed provider or exclusively controlled roots.
