...
- macOS uses a same-process native backend for the current interactive desktop, rejects root and root-owned/loginwindow console sessions, verifies that the process user owns the foreground `/dev/console` session, and fails closed when the session is inactive or locked. Observation includes Screen Recording-gated pixels and an optional independently Accessibility-gated snapshot of the focused application; pointer and keyboard tools remain unavailable.
...
- macOS backend and AX boundary tests: `crates/starweaver-computer-use/src/platform/macos.rs` and `crates/starweaver-computer-use/src/platform/macos_accessibility.rs`
...
- Apple Developer ID signing/notarization and release-byte live Screen Recording/Accessibility permission certification;
