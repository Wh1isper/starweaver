...
`server.mcp_config_path` installs one explicit, strict JSON MCP document. A relative TOML path is resolved against the `rpc.toml` directory; `STARWEAVER_RPC_MCP_CONFIG` overrides it and resolves a relative value against the process working directory. Relative stdio `cwd` values resolve against the MCP document directory. Missing files, unknown fields or transports, transport-mismatched fields, non-string environment/header values, invalid URLs, zero timeouts, duplicate profile selections, unknown servers, and RPC-selected standalone SSE fail startup. RPC supports stdio and streamable HTTP. An empty profile `mcp_servers` selection means every server in the explicit document; a non-empty list is sorted and validated as a set. A server's optional `prefix` changes only model-facing tool names: a non-empty value adds `<prefix>_`, `""` exposes the native MCP name, and omitted/`null` preserves RPC's unprefixed default. The server key and native MCP tool name remain authoritative for selection and invocation, and the static prefix participates in continuation materialization. The legacy `tool_prefix` input spelling remains accepted. Every runtime gets lazy, run-scoped MCP discovery and closes its client on run exit. `init_timeout_ms` bounds connection/discovery, `read_timeout_ms` bounds each MCP tool request, and `exit_timeout_ms` bounds cleanup with a 10-second default. Deferred evidence projects only credential-free transport identity; it never serializes URLs, headers, stdio commands/arguments, working directories, or environments. Lifecycle evidence includes the server/transport, counts, and a credential-free discovered inventory digest. Static MCP configuration participates in continuation materialization, while live inventory is explicitly allowed to drift per run; its digest is diagnostic evidence and `Preserve` does not freeze it. `profile.get` returns effective `mcpServers`; initialize and diagnostics expose the installed config path, and `capabilities.mcp` reports whether a document is installed. The protocol feature `mcp.config_file` identifies vocabulary support independently of that per-process capability. `model.select`, `model.list`, `model.current`, and run profile resolution all use the durable `rpc` client-state scope when `clientStateScope` (or the compatibility `client` alias) is omitted; an explicit scope continues to isolate another client's selection.

### Computer Use composition

When explicitly enabled by RPC-owned `[computer_use]` server configuration, RPC constructs one process-local `starweaver-computer-use` service/router coordinator and automatically materializes the first-party `ComputerUseToolset` into every effective profile. Profiles do not repeat this product-level selection. All admitted runs in that RPC process share the coordinator's native session and serialization fence; each run still requires ordinary initiating-caller and run authorization before handles are attached.

There are no `stdio_observe` or `http_observe` settings and no separate observe, pointer, or keyboard principal capabilities. Enabled configuration plus normal caller/run authorization grants the full canonical family. Maintained RPC composition uses `InputApprovalPolicy::Never`, so pointer and keyboard tools do not add per-call HITL approval metadata. Native Screen Recording and Accessibility/post-event permission remains authoritative.

At every `run.start`, RPC records process-local Computer Use admission bound to `run_id`, a non-secret principal fingerprint, authorization generation, expiry, and immutable config/profile snapshot. It is installed into that run's context but is not serialized as authority. Durable evidence may retain only a redacted admission classification and config snapshot reference. In V1 the authorization generation is immutable process-start evidence and standalone generation `0` is valid; every call compares it with the coordinator's fixed startup value. Runtime configuration revisions are a separate per-run pinned generation, so ordinary profile reload does not revoke an already admitted run while new runs use newly materialized configuration.

Every Computer Use call rechecks the enabled configuration and admitted caller/run generation/expiry, and every effect checks again before the service fence. Revocation or expiry strips handles, cancels queued/active observations and queued/not-yet-delivered effects, and invalidates that run's admission without interrupting unrelated authorized runs. A resume request authenticates and performs normal fresh admission; durable session/run restoration never restores old authority. Automatic/background continuation receives no Computer Use handles unless current managed policy performs the same ordinary admission.

Conformance fixtures MUST prove: disabled configuration or failed normal caller/run authorization exposes no Computer Use tool; enabled authorized runs receive the full family; untrusted profile selection cannot bypass configuration or authorization; two runs with different principals do not share admissions; revocation wins against a queued pre-effect call; maintained tools carry no per-call approval metadata; and resume/continuation without fresh authorization restores no handle.

RPC MUST NOT launch or attach `starweaver-computer-use-mcp` as the implementation of its built-in Computer Use bundle. That stdio binary is the supported boundary for non-Starweaver harnesses; the existing generic MCP client remains a separate facility. Built-in canonical tool-name collisions with configured MCP inventories fail profile validation rather than silently selecting one implementation.

Computer Use is disabled by default. RPC transport reachability, model selection, restored durable state, or an untrusted profile-selection request cannot bypass RPC-owned enabled configuration and ordinary initiating-caller/run authorization. Automatic Toolset materialization does not bypass those gates. An enabled invocation operates only the RPC host process's current active local desktop, never the RPC client's machine. Process-local session handles and observations are not durable, and RPC coordinated shutdown cancels actions, releases input, closes the coordinator, and invalidates them.

This composition adds no Computer-Use-specific JSON-RPC method, protocol feature, path, native selector, or renderer projection. Clients see only ordinary profile/tool behavior already authorized by RPC. The normative architecture and current macOS delivery boundary are [`../computer-use/README.md`](../computer-use/README.md) and [`../alignment/13-computer-use-macos-evidence.md`](../alignment/13-computer-use-macos-evidence.md).
