...
When explicitly enabled by RPC-owned `[computer_use]` server configuration, RPC constructs one process-local `starweaver-computer-use` service/router coordinator and automatically materializes the first-party `ComputerUseToolset` into every effective profile. Profiles do not repeat this product-level selection. All admitted runs in that RPC process share the coordinator's native session and serialization fence; each run still receives independently intersected initiating-caller authorization and per-tool grants before authorized handles are attached.

Generic RPC `run` authorization is necessary but never sufficient for Computer Use. The implemented observe-only release derives a separate transport-principal capability from default-denied `stdio_observe` and `http_observe` configuration; HTTP credentials retain their own principal fingerprint and generic `run` scope grants no observation authority. Future `computer.pointer` and `computer.keyboard` principal capabilities remain statically unavailable and will also require observe.

At every `run.start` admission, RPC derives an in-memory `AdmittedComputerUseRunGrant` as the intersection of process maximum policy, immutable profile/config snapshot, authenticated principal capabilities, requested run/tool policy, and per-tool grants. The record binds `run_id`, a non-secret principal fingerprint, authorization generation, expiry, and the three effective capabilities. It is installed into that run's context but is not serialized as authority. Durable evidence may retain only a redacted grant classification and config snapshot reference. In V1 the authorization generation is immutable process-start evidence and standalone generation `0` is valid; every call compares it with the coordinator's fixed startup value. Runtime configuration revisions are a separate per-run pinned generation, so ordinary profile reload does not revoke an already admitted run while new runs use the newly materialized profile ceiling.

A generic `run` credential or caller-selected profile therefore receives no Computer Use handle without the dedicated caller capability. Every Computer Use call rechecks the admitted grant generation/expiry, and every effect checks again before the service fence. Revocation or expiry strips handles, cancels queued/active observations and queued/not-yet-delivered effects, and invalidates that run's admission without interrupting unrelated authorized runs. A resume request must authenticate again and derive a fresh intersection; durable session/run restoration never restores the old caller grant. Automatic/background continuation receives no Computer Use handles unless a current explicit managed-policy admission performs the same derivation, and V1 attended policy still forbids unattended input.

Conformance fixtures MUST prove: generic `run` alone exposes no Computer Use tool; observe-only cannot obtain pointer/keyboard handles; an untrusted profile selection cannot widen caller capabilities; two runs with different principals do not share grants; revocation wins against a queued pre-effect call; and resume/continuation without fresh authorization restores no handle.
...
Computer Use is disabled by default. RPC transport reachability, an HTTP `run` scope, model selection, restored durable state, or an untrusted profile-selection request cannot exceed RPC-owned server and initiating-caller grants. Automatic Toolset materialization is not caller authority. An enabled invocation operates only the RPC host process's current active local desktop, never the RPC client's machine. Process-local session handles and observations are not durable, and RPC coordinated shutdown cancels actions, releases input, closes the coordinator, and invalidates them.

This composition adds no Computer-Use-specific JSON-RPC method, protocol feature, path, native selector, or renderer projection. Clients see only ordinary profile/tool behavior already authorized by RPC. The normative planned architecture is [`../computer-use/README.md`](../computer-use/README.md).
