...
The same long-lived project key currently signs both Tauri updater artifacts and detached RPC manifests. The private key exists only in release secrets. Desktop embeds only its public key through `STARWEAVER_UPDATE_PUBLIC_KEY`. Development builds without that value remain functional with the bundled RPC but report both update channels as unconfigured.
