...
| Active-run mount changes | host-control active-run attach/detach plus runtime environment handle update and context reinjection          | clients need to add or remove environments after an AgentSession has already started                     | active-run tests prove context refresh, path routing, process routing, and safe conflict handling                 |
