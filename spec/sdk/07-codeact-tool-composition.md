...
The bridge may expose declared target-domain failures as catchable language errors so code can choose an alternate tool or input. Runtime cancellation, exhausted outer deadline, exhausted broker/call budget, lost broker ownership, usage-limit termination, and sandbox-integrity failures are terminal execution signals and cannot be converted by user code into a successful outer result. Whether denial is catchable for branching is executor policy, but it remains a recorded denial and never becomes a successful child outcome.
...
- Large child payloads are not duplicated into outer metadata. Existing artifact-retention and quota rules apply.
