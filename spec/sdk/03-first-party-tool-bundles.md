...
CLI and RPC MUST NOT loop back through their generic MCP client bundle to use `starweaver-computer-use-mcp`; they use the in-process Toolset/library path. An RPC-enabled tool always operates the RPC host process's current desktop, never an RPC client's machine. Generic RPC `run` authority grants nothing: RPC must derive a default-denied, principal-bound, expiring/revocable observe/pointer/keyboard grant for each run and derive it again on resume/continuation before attaching handles. Browser/CDP, provider-native computer tools, remote/VM/unattended/locked-session operation, target/window/PID selection, helpers/daemons, and graphical Desktop ownership are outside this bundle. Every input action is sequential, cites a current observation/geometry basis, and returns a receipt plus a fresh model-visible observation. The library and MCP binary do not persist screenshots or native authority.
