...
Pricing estimates use fixed-point micro USD through `PricingEstimate::amount_micros_usd`, avoiding floats in serialized runtime events. Built-in catalog pricing is best-effort standard direct API pricing. Cache-aware estimates assume `Usage::input_tokens` includes provider-reported cache write/read subtotals and subtract those subtotals before applying cache-specific rates.

`Usage::cache_write_tokens`, `cache_write_1h_tokens`, and `cache_read_tokens` preserve provider-reported evidence; adapters derive a missing aggregate from duration details but do not overwrite a present aggregate. Public construction and external JSON may therefore contain inconsistent values. Derived accounting uses one canonical write-first effective breakdown. `effective_cache_write_tokens()` repairs a one-hour subset larger than its reported aggregate and caps the result at inclusive `input_tokens`; `effective_cache_write_1h_tokens()` is bounded by that aggregate; `effective_cache_read_tokens()` is capped at the input remaining after writes; and `effective_standard_input_tokens()` receives the remainder. For valid `u64` usage, effective write + effective read + effective standard input equals `input_tokens` exactly, including near `u64::MAX`, while snapshots retain the raw evidence.

Pricing charges the effective one-hour subset at `ModelPricingDetails::cache_write_1h_micros_per_million_tokens`, the effective aggregate remainder at the default cache-write rate, effective reads at the read rate, and only the effective standard-input remainder at the input rate. It sums the weighted input-category cost before the single micro-USD ceiling, so equal-rate categories do not gain extra cost from separate rounding. If the one-hour rate is absent, it falls back to the default cache-write rate. Python observability mirrors these effective projections while preserving raw fields in serialized dictionaries.

Future cache categories should retain the aggregate-plus-typed-subset pattern only as a JSON/wire-additive convention. Every new public Rust field remains source-breaking for downstream exhaustive literals and requires explicit propagation through aggregation, snapshots, language projections, pricing, documentation, and tests; release API checks must classify each addition accordingly.

Anthropic adapters normalize `usage.cache_creation_input_tokens` as the aggregate and `usage.cache_creation.ephemeral_1h_input_tokens` as the one-hour subset; when the aggregate is absent, they derive it from the sum of the 5-minute and one-hour breakdown. Built-in Anthropic profiles charge default/5-minute writes at 125% of input, one-hour writes at 200%, and reads at 10%. OpenAI adapters normalize cache-write subtotals from Chat Completions `usage.prompt_tokens_details.cache_write_tokens` and Responses `usage.input_tokens_details.cache_write_tokens`; cache reads come from each protocol's `cached_tokens` detail. GPT-5.6 built-in profiles charge cache writes at 125% and cache reads at 10% of the selected input rate, switching above 272,000 request input tokens to 2x input/cache rates and 1.5x output rates. Context-length tiers are request-scoped: runtime snapshots should add per-request estimates for tiered built-in profiles instead of repricing cumulative run usage.
