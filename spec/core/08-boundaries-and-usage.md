...
`Usage::cache_write_tokens` is the aggregate cache-write count across all durations. Duration-specific counters are typed subsets of that aggregate: `Usage::cache_write_1h_tokens` records one-hour writes and defaults to zero for backward-compatible deserialization. Pricing first charges the bounded one-hour subset at `ModelPricingDetails::cache_write_1h_micros_per_million_tokens`, then charges the aggregate remainder at the default cache-write rate. If the one-hour rate is absent, it falls back to the default cache-write rate without splitting equal-rate rounding. If a producer reports the subset without a valid aggregate, estimation defensively treats the larger value as the effective aggregate. Future cache categories should follow the same additive aggregate-plus-typed-subset pattern rather than changing the meaning of the aggregate field. Adding these public Rust struct fields is intentionally source-breaking for downstream exhaustive literals even though the serialized contract remains backward compatible; release API checks must classify the change accordingly.
