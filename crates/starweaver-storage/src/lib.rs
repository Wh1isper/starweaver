...
use starweaver_stream::{
    DisplayMessage, InMemoryReplayEventLog, ReplayCursor, ReplayError, ReplayEvent,
    ReplayEventKind, ReplayEventLog, ReplayResult, ReplayScope, ReplaySnapshot, ReplaySubscription,
    StreamArchive,
};
...
/// Legacy import preflight options.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct LegacyImportOptions {
    /// Report importable records without mutating the database.
    pub dry_run: bool,
}

/// Legacy backend import preflight report.
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct LegacyImportReport {
    /// True when the report was produced for a dry-run workflow.
    pub dry_run: bool,
    /// Detected source database shape.
    pub source_kind: String,
    /// Shared session store records already present.
    pub existing_shared_sessions: usize,
    /// Shared run store records already present.
    pub existing_shared_runs: usize,
    /// Legacy `SessionStore` session records importable through table preservation.
    pub legacy_store_sessions: usize,
    /// Legacy `SessionStore` run records importable through table preservation.
    pub legacy_store_runs: usize,
    /// Latest backend session records importable into shared session records.
    pub backend_sessions: usize,
    /// Latest backend run records importable into shared run records.
    pub backend_runs: usize,
    /// Backend profiles present in the source database.
    pub profiles: usize,
    /// Backend schedules present in the source database.
    pub schedules: usize,
    /// Backend schedule fires present in the source database.
    pub schedule_fires: usize,
    /// Backend heartbeat fires present in the source database.
    pub heartbeat_fires: usize,
    /// Backend agency fires present in the source database.
    pub agency_fires: usize,
    /// Backend async tasks present in the source database.
    pub async_tasks: usize,
    /// Backend session memory states present in the source database.
    pub memory_states: usize,
    /// Backend bridge conversations present in the source database.
    pub bridge_conversations: usize,
    /// Backend bridge events present in the source database.
    pub bridge_events: usize,
    /// Backend HITL batches present in the source database.
    pub hitl_batches: usize,
    /// Backend HITL interactions present in the source database.
    pub hitl_interactions: usize,
    /// Backend HITL deferred inputs present in the source database.
    pub hitl_deferred_inputs: usize,
    /// Backend bridge HITL messages present in the source database.
    pub bridge_hitl_messages: usize,
    /// Backend workflow definitions present in the source database.
    pub workflow_definitions: usize,
    /// Backend workflow runs present in the source database.
    pub workflow_runs: usize,
    /// Backend workflow node runs present in the source database.
    pub workflow_node_runs: usize,
    /// Backend workflow events present in the source database.
    pub workflow_events: usize,
    /// Total session records that migration can import into shared contracts.
    pub importable_sessions: usize,
    /// Total run records that migration can import into shared contracts.
    pub importable_runs: usize,
    /// Preflight warnings for operators.
...
/// Analyze legacy and latest-backend importable records without applying migrations.
///
/// # Errors
///
/// Returns a store error when SQLite cannot open or inspect the database.
pub fn analyze_legacy_import(
    path: impl AsRef<Path>,
    options: LegacyImportOptions,
) -> SessionStoreResult<LegacyImportReport> {
    let connection = Connection::open(path).map_err(sql_error)?;
    analyze_legacy_import_for_connection(&connection, options)
}

/// Build a dry-run legacy import report.
///
/// # Errors
///
/// Returns a store error when SQLite cannot open or inspect the database.
pub fn dry_run_legacy_import(path: impl AsRef<Path>) -> SessionStoreResult<LegacyImportReport> {
    analyze_legacy_import(path, LegacyImportOptions { dry_run: true })
}
...
fn load_applied_migration_records(
    ...
    let mut statement = connection
        .prepare(&format!(
            "SELECT id, description, applied_at FROM {SQLITE_SCHEMA_MIGRATION_TABLE} ORDER BY applied_at ASC, id ASC"
        ))
        .map_err(sql_error)?;
    let rows = statement
        .query_map([], |row| {
...
fn analyze_legacy_import_for_connection(
    connection: &Connection,
    options: LegacyImportOptions,
) -> SessionStoreResult<LegacyImportReport> {
    let existing_shared_sessions = count_table_rows_if_exists(connection, "session_records")?;
    let existing_shared_runs = count_table_rows_if_exists(connection, "run_records")?;
    let legacy_store_sessions = if has_legacy_session_record_table(connection)? {
        count_table_rows_if_exists(connection, "sessions")?
    } else {
        count_table_rows_if_exists(connection, "legacy_session_records")?
    };
    let legacy_store_runs = if has_legacy_run_record_table(connection)? {
        count_table_rows_if_exists(connection, "runs")?
    } else {
        count_table_rows_if_exists(connection, "legacy_run_records")?
    };
    let backend_sessions = if has_backend_session_table(connection)? {
        count_table_rows_if_exists(connection, "sessions")?
    } else {
        0
    };
    let backend_runs = if has_backend_run_table(connection)? {
        count_table_rows_if_exists(connection, "runs")?
    } else {
        0
    };
    let mut report = LegacyImportReport {
        ...
        existing_shared_sessions,
        existing_shared_runs,
        legacy_store_sessions,
        legacy_store_runs,
        backend_sessions,
        backend_runs,
        profiles: count_table_rows_if_exists(connection, "profiles")?,
        schedules: count_table_rows_if_exists(connection, "schedules")?,
        schedule_fires: count_table_rows_if_exists(connection, "schedule_fires")?,
        heartbeat_fires: count_table_rows_if_exists(connection, "heartbeat_fires")?,
        agency_fires: count_table_rows_if_exists(connection, "agency_fires")?,
        async_tasks: count_table_rows_if_exists(connection, "session_async_tasks")?,
        memory_states: count_table_rows_if_exists(connection, "session_memory_states")?,
        bridge_conversations: count_table_rows_if_exists(connection, "bridge_conversations")?,
        bridge_events: count_table_rows_if_exists(connection, "bridge_events")?,
        hitl_batches: count_table_rows_if_exists(connection, "hitl_batches")?,
        hitl_interactions: count_table_rows_if_exists(connection, "hitl_interactions")?,
        hitl_deferred_inputs: count_table_rows_if_exists(connection, "hitl_deferred_inputs")?,
        bridge_hitl_messages: count_table_rows_if_exists(connection, "bridge_hitl_messages")?,
        workflow_definitions: count_table_rows_if_exists(connection, "workflow_definitions")?,
        workflow_runs: count_table_rows_if_exists(connection, "workflow_runs")?,
        workflow_node_runs: count_table_rows_if_exists(connection, "workflow_node_runs")?,
        workflow_events: count_table_rows_if_exists(connection, "workflow_events")?,
        importable_sessions: legacy_store_sessions.saturating_add(backend_sessions),
        importable_runs: legacy_store_runs.saturating_add(backend_runs),
        warnings: Vec::new(),
    };
    report.source_kind = legacy_source_kind(&report);
    if backend_sessions > 0 && backend_runs == 0 {
        report
            .warnings
            .push("latest backend sessions were found without importable runs".to_string());
    }
    if backend_runs > 0 && backend_sessions == 0 {
        report
            .warnings
            .push("latest backend runs were found without importable sessions".to_string());
    }
    if legacy_store_sessions > 0 && backend_sessions > 0 {
        report.warnings.push(
            "legacy SessionStore records and latest backend sessions are both present".to_string(),
        );
    }
    Ok(report)
}

fn legacy_source_kind(report: &LegacyImportReport) -> String {
    let has_shared = report.existing_shared_sessions > 0 || report.existing_shared_runs > 0;
    let has_legacy = report.legacy_store_sessions > 0 || report.legacy_store_runs > 0;
    let has_backend = report.backend_sessions > 0 || report.backend_runs > 0;
    match (has_shared, has_legacy, has_backend) {
        (false, false, false) => "empty",
        (true, false, false) => "shared_store",
        (false, true, false) => "legacy_session_store",
        (false, false, true) => "latest_backend",
        _ => "mixed",
    }
    .to_string()
}
...
fn count_table_rows_if_exists(connection: &Connection, table: &str) -> SessionStoreResult<usize> {
...
mod tests {
    ...
    #[test]
    fn sqlite_migration_status_and_import_dry_run_are_reported() {
        let tempdir = tempfile::tempdir().expect("tempdir");
        ...
        let import_path = tempdir.path().join("import-dry-run.sqlite3");
        let connection = Connection::open(&import_path).expect("open import database");
        connection
            .execute_batch(
                r"
                CREATE TABLE sessions (id TEXT PRIMARY KEY);
                CREATE TABLE runs (id TEXT PRIMARY KEY, input_parts TEXT NOT NULL DEFAULT '[]');
                CREATE TABLE profiles (name TEXT PRIMARY KEY);
                CREATE TABLE schedules (id TEXT PRIMARY KEY);
                CREATE TABLE session_async_tasks (id TEXT PRIMARY KEY);
                CREATE TABLE session_memory_states (source_session_id TEXT PRIMARY KEY);
                CREATE TABLE workflow_definitions (id TEXT PRIMARY KEY);
                INSERT INTO sessions (id) VALUES ('session_backend');
                INSERT INTO runs (id, input_parts) VALUES ('run_backend_1', '[]'), ('run_backend_2', '[]');
                INSERT INTO profiles (name) VALUES ('default');
                INSERT INTO schedules (id) VALUES ('schedule_1');
                INSERT INTO session_async_tasks (id) VALUES ('task_1');
                INSERT INTO session_memory_states (source_session_id) VALUES ('session_backend');
                INSERT INTO workflow_definitions (id) VALUES ('workflow_1');
                ",
            )
            .expect("seed import database");
        drop(connection);

        let report = dry_run_legacy_import(&import_path).expect("dry-run report");
        assert!(report.dry_run);
        assert_eq!(report.source_kind, "latest_backend");
        assert_eq!(report.backend_sessions, 1);
        assert_eq!(report.backend_runs, 2);
        assert_eq!(report.profiles, 1);
        assert_eq!(report.schedules, 1);
        assert_eq!(report.async_tasks, 1);
        assert_eq!(report.memory_states, 1);
        assert_eq!(report.workflow_definitions, 1);
        assert_eq!(report.importable_sessions, 1);
        assert_eq!(report.importable_runs, 2);
    }
...
#[cfg(test)]
mod replay_tests {
    use starweaver_runtime::AgentStreamEvent;
