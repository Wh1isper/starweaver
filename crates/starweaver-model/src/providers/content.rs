...
use crate::{ModelError, message::ContentPart, parse_data_url};
...
pub fn openai_chat_content_with_cache_points(
    content: &[ContentPart],
    enable_cache_points: bool,
        ...
        if let ContentPart::CachePoint { ttl } = part {
            ...
            if ttl.is_some() {
                return Err(ModelError::MessageMapping(
                    "OpenAI GPT-5.6 cache points use request-wide 30m TTL; omit the per-point TTL"
                        .to_string(),
                ));
            }
            let previous = blocks.last_mut().ok_or_else(|| {
                ModelError::MessageMapping(
                    "cache point cannot be the first content in an OpenAI message".to_string(),
                )
            })?;
            previous["prompt_cache_breakpoint"] = json!({"mode": "explicit"});
            continue;
        }
...
pub fn openai_responses_content_with_cache_points(
    content: &[ContentPart],
    enable_cache_points: bool,
...
pub fn gemini_parts_from_content(content: &[ContentPart]) -> Vec<Value> {
    content
        ...
        .filter_map(|part| match part {
            ...
            ContentPart::FileUrl { url, media_type } => Some(json!({
                "fileData": {"fileUri": url, "mimeType": media_type}
            })),
            ContentPart::Binary { data, media_type } => Some(json!({
            ...
            ContentPart::DataUrl {
                data_url,
                media_type,
            } => Some(gemini_data_url(data_url, media_type)),
        })
        .collect()
}
...
pub fn bedrock_content_from_content(content: &[ContentPart]) -> Vec<Value> {
    content
        ...
        .filter_map(|part| match part {
            ...
            ContentPart::Text { text } => Some(json!({"text": text})),
            ContentPart::ImageUrl { url } => {
                Some(json!({"image": {"source": {"bytes": url}}}))
            }
            ContentPart::FileUrl { url, media_type } => Some(json!({
                "document": {
                    "format": media_type,
            ...
            ContentPart::Binary { data, media_type } => Some(if media_type.starts_with("image/") {
            ...
            ContentPart::DataUrl {
                data_url,
                media_type,
            } => Some(bedrock_data_url(data_url, media_type)),
        })
        .collect()
}
