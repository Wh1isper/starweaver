...
- `crates/starweaver-rpc-core` — typed JSON-RPC host protocol contracts, envelopes, errors, stream payload projection, and replay result helpers; `spec/ops/09-rpc-idl-and-client-generation.md` defines the planned migration of canonical wire types to generated Rust bindings
- `crates/starweaver-rpc` — independent standalone JSON-RPC host product for local and external host integrations, owning `rpc.toml`, AgentSpec/profile/model materialization, handlers, coordination, authorization, subscriptions, environment attachments, and transports; it will implement the generated Rust server boundary after the IDL migration
...
- `spec/ops/09-rpc-idl-and-client-generation.md` — OpenRPC/JSON Schema structural source of truth, generated Rust server boundary, safe manifest-filtered TypeScript Desktop bridge/client, compatibility, migration, and validation
