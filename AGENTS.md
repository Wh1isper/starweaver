...
- `crates/starweaver-rpc-core` — current handwritten major-1 JSON-RPC host contracts plus framing/projection helpers; `spec/ops/09-rpc-idl-and-client-generation.md` defines an independent generated major-2 wire boundary
- `crates/starweaver-rpc` — independent standalone JSON-RPC host product for local and external host integrations, owning `rpc.toml`, AgentSpec/profile/model materialization, handlers, coordination, authorization, subscriptions, environment attachments, and transports; it will implement the generated major-2 server boundary while isolating any retained major-1 dispatch
...
- The accepted next host protocol is `starweaver.host` major 2: JSON-RPC 2.0 with a checked-in OpenRPC 1.4.0/JSON Schema Draft 7 IDL as the structural wire source of truth. It uses non-empty string client request IDs, required object params, canonical decimal-string 64-bit values, explicit supported/required feature negotiation, root-registered typed public errors, and one durable replay/live `EventRecord` vocabulary with atomic state/outbox publication, opaque scope/view-bound cursors, and identical feature/authorization eligibility. The IDL generates Rust server bindings in `starweaver-rpc-core` and manifest-filtered safe TypeScript bridge/client bindings for Desktop; neither generated language surface may become a competing protocol definition. Current Rust DTOs and the v1 corpus remain authoritative only for implemented major 1 and are behavioral inventory, not major-2 wire constraints.
- `starweaver-rpc-core`: handwritten major-1 JSON-RPC contracts plus narrow framing/projection helpers while v1 is supported; major-2 wire types, validators, server trait, and dispatcher are generated without moving product behavior into this crate.
- `starweaver-rpc`: independent standalone JSON-RPC host product and future implementer of the generated major-2 server boundary; it owns `rpc.toml`, profile/model materialization, handlers, and active-run state, must not depend on CLI, and independently connects to shared storage, environment, and envd abstractions.
- Starweaver Desktop execution will require the IDL-first host major 2 and use manifest-filtered safe TypeScript bridge/client bindings behind a Tauri 2 privileged Rust backend and a renderer restricted to generated safe operation DTOs. The Rust backend retains child/SSH transport, request IDs, idempotency keys, routing, replay recovery, wire construction, safe output projection, and authority; the renderer must not import the complete generated host model or `HostRpcClient`, submit free-form JSON-RPC or complete host params, or provide supervisor-owned fields. The implemented shell foundation owns process state and application-owned single-instance transports that carry only a fixed activation frame; argv and working directories must not be read or forwarded. Renderer activation uses a dedicated typed IPC channel, never a general Tauri event-listener grant. The foundation intentionally has no RPC/storage/OAuth/environment/SSH authority yet. Future execution supervises local stdio RPC children and system-OpenSSH-carried remote RPC processes keyed by a stable execution-domain binding plus canonical workspace, with least-authority catalog paths per storage domain. SSH v1 uses a generated allowlisted OpenSSH configuration, separates mutable host-key/runtime evidence from routing identity, and requires a storage-owned remote OS-held lock under a non-overridable platform-canonical per-user coordination root, keyed by stable database/workspace identities and independent of config/child state directories, plus a fenced owner generation before a process gains execution authority. Desktop must not link CLI, RPC host, agent, runtime, or storage implementations, parse CLI private configuration as a runtime dependency, read local/remote SQLite or OAuth files, expose SSH to the renderer, or send arbitrary renderer RPC. Desktop launches hosts only through a public versioned RPC launch envelope, not private `rpc.toml`; remote login-shell startup uses a bounded nonce-marked supervised bootstrap before strict JSON-RPC. Local per-workspace processes are not an OS sandbox, so native local shell is disabled by default without an enforceable sandbox. An explicitly granted SSH domain may enable native remote shell with full authenticated remote-account authority, which must never be represented as workspace containment. The supervisor owns verified, compatibility-gated local and remote RPC runtime updates; remote provisioning may invoke only the public exact-version non-interactive RPC component contract shared with the hardened `starweaver`/`sw` updater, never CLI-private handlers or configuration.
...
- `spec/ops/09-rpc-idl-and-client-generation.md` — clean IDL-first JSON-RPC major 2, OpenRPC/JSON Schema structural source, generated Rust server boundary, safe manifest-filtered TypeScript Desktop bridge/client, compatibility, cutover, and validation
...
- `spec/desktop/07-ssh-remote-workspaces.md` — SSH execution domains, login-shell RPC bootstrap, remote account authority, component provisioning/update, and reconnect
