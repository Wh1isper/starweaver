...
- Starweaver Desktop's first release is the local product itself, not a dogfooding edition. Use local product, local release, or prerelease terminology as appropriate; do not label the product or its UX as dogfood/dogfooding.
...
- `apps/starweaver-desktop/src-tauri` — Tauri 2 Desktop shell with process-owned state, single-instance activation, least-authority renderer IPC, verified adjacent local RPC supervision, backend-owned workspace grants and conversation-window routes, window-owned replay subscriptions, domain-scoped workspace/session/run routing, durable safe transcript delivery, and Linux/macOS/Windows build gates; every native package retains an exact RPC sidecar as bootstrap/fallback, while the first release supports project-signed exact-contract RPC selection/rollback for the next process start and Rust-only Tauri Desktop install/restart; OS publisher signing remains intentionally unconfigured
...
This pushes `release/vX.Y.Z` for review. After the release commit reaches `main`, publish `vX.Y.Z` as a GitHub Release. The `release.yml` workflow runs from the published Release event with independent core, signed RPC runtime, and Desktop publication lanes. Desktop build/signing/metadata failure must remain visible but must not block CLI/protocol assets, crates.io, PyPI, or independently signed runtime assets. Core assets use `checksums.txt`, runtime assets use `runtime-checksums.txt`, and Desktop assets use `desktop-checksums.txt`. Release assets are immutable: automation refuses name collisions and publishes payloads before checksum/updater metadata rather than using `--clobber`.
