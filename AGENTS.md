...
- `apps/starweaver-desktop/src-tauri` — Tauri 2 Desktop shell with process-owned state, single-instance activation, least-authority renderer IPC, verified adjacent local RPC supervision, backend-owned workspace grants and conversation-window routes, window-owned replay subscriptions, domain-scoped workspace/session/run routing, durable safe transcript delivery, and Linux/macOS/Windows build gates; updater-owned signed runtime selection remains a later milestone
