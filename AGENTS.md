...
- `crates/starweaver-computer-use` — typed current-active-desktop service/state machine, canonical tool schemas/router, deterministic fake, macOS pixel plus bounded Accessibility observe-only backend, explicit unsupported backends, and optional feature-gated `starweaver-computer-use-mcp` stdio binary
...
- `docs/computer-use.md` — macOS pixel and Accessibility observe-only support, CLI/RPC opt-in, permissions, external MCP use, and platform limits
...
- `spec/computer-use/README.md` — current-active-desktop Computer Use boundary, implemented macOS pixel and bounded Accessibility observe-only subset, fixed exclusions, package shape, and reading order
...
- `spec/alignment/13-computer-use-macos-evidence.md` — macOS pixel and bounded Accessibility observe-only Computer Use implementation, security boundary, release integration, and validation evidence
