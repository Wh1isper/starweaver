...
A CLI-owned run currently commits raw/display evidence atomically when it completes, fails, is cancelled, or enters a durable waiting state. Therefore RPC can replay or subscribe to the complete retained backlog, but it does not observe token/display deltas while that CLI process is still executing the run. Real-time Desktop observation of an active terminal-owned run requires a future incremental publication contract; clients must not infer that behavior from `stream.subscribe`.
